...
default_args = {
    "owner": "airflow",
    "start_date": datetime(2025, 2, 2),  # 5 days in the past
    "retries": 1,
}
...
@dag(
    ...
    schedule_interval=timedelta(days=2),  # Every 2 days
    catchup=True,
)
def schedule_dag():
    ...
    def process_data():
        ...
        df_filtered.to_csv(f"{base_path}/processed/yelp.csv", index=False)
    ...
    cleanup = BashOperator(
        task_id="cleanup",
        bash_command="rm -rf ${AIRFLOW_HOME}/downloads/yelp.csv",
    )
